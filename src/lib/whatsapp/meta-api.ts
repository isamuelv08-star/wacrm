...
/**
 * fetch() with a timeout. None of the Graph API calls had one, so a
 * hung Meta request stalled the whole webhook batch (and everything
 * queued behind it in `after()`) indefinitely. A caller-supplied
 * signal wins, for the longer media upload.
 */
const META_TIMEOUT_MS = 30_000
const META_UPLOAD_TIMEOUT_MS = 120_000
function metaFetch(input: string, init: RequestInit = {}): Promise<Response> {
  return fetch(input, { signal: AbortSignal.timeout(META_TIMEOUT_MS), ...init })
}
