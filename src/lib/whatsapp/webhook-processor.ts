...
//
// Extracted from the Meta-direct webhook route so a second route
// (Coexistence providers like Dualhook, which front the Meta
// connection and therefore sign payloads with THEIR OWN app secret
// instead of ours) can reuse the exact same message-processing logic
// without duplicating it. Each route owns its own auth — HMAC
// signature verification for direct Meta connections, a URL secret
// for Dualhook — and both call `processWebhookPayload` once the
...
// ============================================================
// GET verification — shared by every webhook route (Meta-direct and
// Dualhook alike). Meta's `hub.verify_token` challenge has nothing to
// do with which base URL or signing secret a route uses, so both
// routes hand this the same check against whatsapp_config.verify_token.
// ============================================================
export async function handleWebhookVerificationGET(request: Request): Promise<Response> {
  try {
...
// ============================================================
// POST payload processing — shared by every webhook route once its
// own auth (HMAC signature for Meta-direct, URL secret for Dualhook)
// has accepted the request.
// ============================================================
export async function processWebhookPayload(
  body: { entry?: WhatsAppWebhookEntry[] },
  options: {
    /** Dualhook shares one URL secret across every tenant, so it may
     *  only feed numbers actually connected through Coexistence
     *  (send_api_base set) — never a Meta-direct number whose events
     *  are HMAC-verified on the other route. */
    coexistenceOnly?: boolean
  } = {},
) {
  ...
  for (const entry of body.entry) {
    for (const change of entry.changes ?? []) {
      // One bad change/message must not drop the rest of the batch:
      // we've already acked 200, so Meta won't redeliver anything.
      try {
        await processWebhookChange(change, options)
      } catch (err) {
        console.error('[webhook] failed to process change:', err)
      }
    }
  }
}
...
async function processWebhookChange(
  ...
  if (options.coexistenceOnly && !config.send_api_base) {
    console.warn(
      '[webhook] dualhook payload for a non-Coexistence number ignored:',
      phoneNumberId,
    )
    return
  }
  ...
  try {
    ...
    console.error('[webhook] cannot decrypt access token for config', config.id, err)
    return
  }
