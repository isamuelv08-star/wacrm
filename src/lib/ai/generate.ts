...
export function parseGeneration(
  ...
  const sendQuoteMatch = raw.match(SEND_QUOTE_SENTINEL_PATTERN)
  const sendQuote = sendQuoteMatch ? sendQuoteMatch[1].trim() || null : null
  ...
  const text = raw
    ...
    .replace(SEND_QUOTE_SENTINEL_PATTERN, '')
