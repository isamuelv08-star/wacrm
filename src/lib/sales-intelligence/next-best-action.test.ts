...
import { buildNextBestActions, NEXT_BEST_ACTION_LIMIT, type OverduePromise } from './next-best-action'
...
function overduePromise(overrides: Partial<OverduePromise> = {}): OverduePromise {
  return {
    id: 'promise-1',
...
describe('buildNextBestActions', () => {
  it('returns nothing for three empty inputs', () => {
  ...
  it('turns an overdue promise into a fulfill_broken_promise action, keyed by promiseId not dealId', () => {
    const [action] = buildNextBestActions([], [], [overduePromise({ dueAt: daysAgo(3) })], NOW)
    expect(action.type).toBe('fulfill_broken_promise')
    expect(action.id).toBe('promise-1')
    expect(action.dealId).toBeNull()
    expect(action.promiseId).toBe('promise-1')
    expect(action.daysInactive).toBe(3)
    expect(action.value).toBeNull()
  })

  it('never drops a broken-promise action for lacking a matching deal — it has its own id space', () => {
    const actions = buildNextBestActions(
      [stalled({ id: 'deal-x' })],
      [atRisk({ id: 'deal-y' })],
      [overduePromise({ id: 'promise-z' })],
      NOW,
    )
    expect(actions.map((a) => a.type).sort()).toEqual(
      ['follow_up_stalled', 'fulfill_broken_promise', 're_engage_silent'].sort(),
    )
  })

  it('escalates broken-promise urgency with days overdue', () => {
    const low = buildNextBestActions([], [], [overduePromise({ id: 'p1', dueAt: daysAgo(0) })], NOW)[0]
    const medium = buildNextBestActions([], [], [overduePromise({ id: 'p2', dueAt: daysAgo(1) })], NOW)[0]
    const high = buildNextBestActions([], [], [overduePromise({ id: 'p3', dueAt: daysAgo(3) })], NOW)[0]
    expect(low.urgency).toBe('low')
    expect(medium.urgency).toBe('medium')
    expect(high.urgency).toBe('high')
  })
