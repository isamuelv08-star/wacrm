...
export interface DealFormFieldsProps {
   ...
   *  gates the field reset and the supporting-data fetch, same role
   *  `DealForm`'s old `open` prop played on its Sheet. */
  open: boolean;
  deal?: Deal | null;
  pipelineId: string;
  ...
  onSaved: () => void;
  /** Save, cancel, and delete all call this — "I'm done, whatever is
   *  hosting me should go away now" — instead of an `onOpenChange`,
   *  so this same component works standalone in a Sheet (DealForm) or
   *  embedded in a tab (LeadSummarySheet's "Edit" tab) without either
   *  caller needing to know about the other. */
  onClose: () => void;
}

/**
 * The actual deal editor — every field, save/cancel, delete, and the
 * "schedule a follow-up" shortcut. Extracted out of `DealForm`'s Sheet
 * chrome so LeadSummarySheet can embed it directly as a tab (edit
 * without leaving the lead's summary, tabs side by side — see that
 * component's doc comment) while `DealForm` keeps working exactly as
 * before for its other call site (contact-sidebar.tsx's deal click).
 */
export function DealFormFields({
  ...
  // Reset the form fields every time this becomes visible or its input
  ...
  // Load supporting data once visible
  useEffect(() => {
    if (!open) return;
    let cancelled = false;
    (async () => {
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="themed-scrollbar flex-1 overflow-y-auto p-4 space-y-4">
