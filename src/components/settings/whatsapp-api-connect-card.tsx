 ...
 * same status/dialog logic:
 *   - `variant="row"` (default) — title + hint on the left, a
 *     status/action button on the right. Mirrors
 *     <ConnectPlatformButton>'s layout so onboarding's linear list of
 *     WhatsApp options reads as siblings, not two different UIs
 *     bolted together.
 *   - `variant="card"` — a tile for the Settings → Integrations grid
 *     (see <IntegrationCard>).
 * Clicking the button opens the full credentials form (unchanged —
 * still <WhatsAppConfig />, just relocated into a dialog instead of
 * always sitting inline on the page) rather than permanently taking
 * up page space.
 */
export function WhatsAppApiConnectCard({ variant = 'row' }: { variant?: 'row' | 'card' }) {
