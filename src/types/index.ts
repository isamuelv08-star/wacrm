...
export interface Contact {
  ...
  /**
   * Messenger's page-scoped user id (migration 080) — set instead of a
   * real value in `phone` for a contact that arrived through Messenger,
   * which has no phone number. `phone` itself stays a stable, non-empty
   * placeholder (`"Messenger:<psid>"`) rather than null, since most of
   * the app still treats `phone` as always-present; render `name` (or
   * this field's presence) rather than `phone` wherever a Messenger
   * contact's identity is shown.
   */
...
export interface Conversation {
  ...
  /**
   * Source channel for the conversation — a real column since migration
   * 080 (Messenger). Instagram is still listed for forward-compatibility
   * with the Inbox's per-platform UI (see {@link getConversationPlatform})
...
export interface MessengerConfig {
  ...
  /** Last error from /subscribed_apps; cleared on success. */
  last_subscribe_error?: string | null;
}
