...
export async function PATCH(
  ...
  try {
    ...
    // Editing/deleting a template changes it at Meta for the whole
    // business — admin+ only. getUser() alone let any viewer/agent do it
    // (RLS only blocked the local row write, after Meta already changed).
    ...
    if (!isDryRun()) {
      // Same stopgap as templates/submit and templates/sync — always
      // the oldest connected number for a multiwhatsapp account (085).
...
export async function DELETE(
  ...
  try {
    ...
    if (existing.meta_template_id && !isDryRun()) {
      // Same stopgap as above — always the oldest connected number for
      // a multiwhatsapp account (085).
      const { data: configRows, error: configError } = await supabase
        .from('whatsapp_config')
        .select('*')
        .eq('account_id', accountId)
